...
I have assigned this dataset to the object `brown_fam_df` and have made minor modifications to the variable names in @lst-brown-curate to improve the readability of the dataset.
...
Let's create a statistical overview of the `brown_fam_df` dataset in @lst-skim-brown-fam.
...
Let's extract the statistical overview of the numeric variables in the `brown_fam_df` dataset in @lst-brown-yank-num.
...
For example, let's calculate the mean and median of the `percent_passive` variable in the `brown_fam_df` dataset grouped by the `lang_variety` variable, as seen in @lst-brown-summarize-lang-variety.

::: {#lst-brown-summarize-lang-variety}
```{r}
...
The `group_by()` function can also be used to group by multiple variables. For example, in @lst-brown-summarize-grouped we calculate the mean and median of the `percent_passive` variable in the `brown_fam_df` dataset grouped by the `lang_variety` and `genre` variables.

::: {#lst-brown-summarize-grouped}
```{r}
...
For character variables, and factors, the summary statistics are more limited. We can calculate the number of observations (`n()`) and/ or the number of unique values (`n_distinct()`). Let's now summarize the number of observations `n()` grouped by the `genre` variable in the `brown_fam_df` dataset in @lst-summarize-brown-genre-n-distinct.

::: {#lst-summarize-brown-genre-n-distinct}
```{r}
...
Just as before, we can add multiple grouping variables to `group_by()`. Let's add `lang_variety` to the grouping and calculate the number of observations `n()` grouped by the `genre` and `lang_variety` variables in the `brown_fam_df` dataset as seen in @lst-summarize-brown-genre-lang-variety-n-distinct.

::: {#lst-summarize-brown-genre-lang-variety-n-distinct}
```{r}
...
If we do not use the `ungroup()` function, the mean and median will be calculated for each `genre` collapsed across `lang_variety`. We can see this in @lst-summarize-brown-genre-n-distinct-mean-median.

::: {#lst-summarize-brown-genre-n-distinct-mean-median}
```{r}
...
If we use the `ungroup()` function, the mean and median will be calculated for all genres. Note we will use the `ungroup()` function between these summaries to clear the grouping before calculating the mean and median, as in @lst-summarize-ungroup-mean-median.

::: {#lst-summarize-ungroup-mean-median}
```{r}
...
```
1. Use the `ungroup()` function to remove the grouping before calculating the mean and median.
:::
...
<!-- TODO: continue adding snippet blocks ... -->
