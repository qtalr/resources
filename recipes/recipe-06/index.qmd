...
In this recipe, we will make use of {readr}, {dplyr}, {stringr}, and {purrr}, employ regular expressions to parse the semi-structured data, and use {qtkit} to document the dataset. Let's load those packages now.
...
# [ ] add to {qtkit}, note the convention of `extract_` prefix for curation functions. In combination with `get_archive_data()` this corpus can be curated with few steps.

curate_swda_data <- function(file) {
  # Progress message
...
As a sanity check we will run the `curate_swda_data()` function on a the conversation file we were just working on to make sure it works as expected.
...
To pass each conversation file in the vector of paths to our conversation files iteratively to the `curate_swda_data()` function we use `map_dfr()`. This will apply the function to each conversation file and return a data frame for each and then combine the results into a single data frame.
...
swda_df <-
  ...
  map_dfr(curate_swda_data) # read and tidy iteratively
...
The data dictionary file will contain information about the dataset variables and their values. This file can be created manually and edited with a text editor or spreadsheet software. Or alternatively, the scaffolding for a CSV file can be generated with the `create_data_dictionary()` function from {qtkit}.
...
The skills we used in this recipe include regular expressions, the {readr}, {dplyr}, {stringr}, and {purrr}, and {qtkit} for documenting the dataset.
